...
/*   Updated: 2023/07/18 09:51:31 by ouaarabe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*fill_ns_hd(char *cmdl, int count, t_quote cq, t_hd hd, int indx)
{
	char	*new_s;
	int		j;
	int		z;
...
char	**ns_heredoc(char **cmdl, int wc)
{
	...
	while (cmdl[hd.i])
	{
		...
		if (hd.print)
		{
			new_s[hd.k++] = fill_ns_hd(cmdl[hd.z], hd.count, cq, hd, hd.z);
			hd.print = 0;
			hd.count = 0;
		}
		hd.i++;
